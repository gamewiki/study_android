...
//       System.out.println("Wrong month, ^_^ , i think you are want a -1, go to death!");
   ...
   // The lunar calendar is turned into the Solar calendar
   public static PersonDestiny getPersonByCalendarLundarToSolar(int iYear, int iMonth, int iDay,PersonDestiny person) {
     int iSYear, iSMonth, iSDay;
     int iOffsetDays = iGetLNewYearOffsetDays(iYear, iMonth, iDay) + iSolarLunarOffsetTable[iYear - 1901];
     int iYearDays = bIsSolarLeapYear(iYear) ? 366 : 365;
     if (iOffsetDays >= iYearDays) {
     ...
     iSMonth--;
     person.setYear(iSYear);
     person.setMonth(iSMonth);
     person.setDay(iSDay);
//     System.out.println("" + iSYear + (iSMonth > 9 ? iSMonth + "" : "0" + iSMonth) + (iSDay > 9 ? iSDay + "" : "0" + iSDay));
	return person;
   }

	public static void main(String[] args) {
//		System.out.println(ChineseCalendar.sCalendarLundarToSolar(1988, 11, 12));
	}
}
// 自定义星期类
class Week {
