...
public class Sendding extends Activity {
	private int id = 0;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		id = getIntent().getIntExtra("id", 0);
		super.onCreate(savedInstanceState);
		Clock clock = DatabaseOperation.getClock(Sendding.this,id);
		createNotifaction(clock);
		/* 跳出的闹铃警示 */
		Builder dialog = new AlertDialog.Builder(Sendding.this);
	...
	class cancelClockListener implements DialogInterface.OnClickListener {
		...
		public void onClick(DialogInterface dialog, int which) {
			if (id!=0) {
				AlarmHelper ah = new AlarmHelper(Sendding.this);
				ah.closeAlarm(id);
